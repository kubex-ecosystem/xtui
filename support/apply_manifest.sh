...
__replace_project_name() {
  ...
  tar --exclude='bkp' --exclude='*.tar.gz' --exclude='go.sum' -czf "$_ROOT_DIR/bkp/$(date +%Y%m%d_%H%M%S)_goforge_backup.tar.gz" -C "$_ROOT_DIR" . || {
    log fatal "Could not create backup. Please check if the directory exists and is writable."
    return 1
  }
  ...
  for _file in "${_files_to_remove[@]}"; do
    if [[ -f $_file ]]; then
      rm -f "$_file" || {
        log error "Could not remove $_file. Please check if the file exists and is writable."
        continue
      }
  ...
  for _file in "${_files_to_rename[@]}"; do
    if [[ -f $_file ]]; then
      ...
      mv "$_file" "$_new_file" || {
        log error "Could not rename $_file to $_new_file. Please check if the file exists and is writable."
        continue
      }
  ...
  for _file in "${_files_to_update[@]}"; do
    if [[ -f $_file ]]; then
      sed -i "s/$_old_bin_name/$_new_bin_name/g" "$_file" || {
        log error "Could not update $_file. Please check if the file exists and is writable."
        continue
      }
  ...
  cd "$_ROOT_DIR" || {
    log error "Could not change directory to $_ROOT_DIR. Please check if the directory exists."
    return 1
  }

  go mod tidy || {
    log error "Could not run 'go mod tidy'. Please check if Go is installed and configured correctly."
    return 1
  }
