...
validate_go_version() {
  ...
  if ! check_go_version_compatibility "${_GO_VERSION:-}"; then
...
load_platforms_from_manifest() {
  ...
  if [[ -f ${_manifest_path} ]] && have jq; then
    local platforms=()
...
discover_main_packages() {
  ...
  mapfile -t _main_dirs_arr < <(go list -f '{{if eq .Name "main"}}{{.Dir}}{{end}}' "${_target_path}" 2>/dev/null | awk 'NF' | grep -v swagger)
  ...
  log info "Found ${#_main_dirs_arr[@]} main package(s): ${_main_dirs_arr[*]}" >&2
  printf '%s\n' "${_main_dirs_arr[@]}"
}
...
compile_binary() {
  ...
  local _build_env=(
    "GOOS=${_platform_pos}"
    "GOARCH=${_arch_pos}"
    "CGO_ENABLED=${_CGO_ENABLED:-${CGO_ENABLED:-0}}"
  )

  if [ "${_platform_pos}" == "windows" ]; then
    if ! have x86_64-w64-mingw32-gcc; then
      log warn "x86_64-w64-mingw32-gcc not found, trying with gcc with no guarantee of success"
      log info "To install x86_64-w64-mingw32-gcc on Debian/Ubuntu, run: sudo apt install gcc-mingw-w64"
      log info "To install x86_64-w64-mingw32-gcc on Fedora, run: sudo dnf install mingw-gcc-x86_64-linux-gnu"
      log info "To install x86_64-w64-mingw32-gcc on Arch Linux, run: sudo pacman -S mingw-w64-x86_64-gcc"
      log info "To install x86_64-w64-mingw32-gcc on macOS, run: brew install mingw-w64"
      _build_env+=("CC=gcc")
    else
      _build_env+=("CC=x86_64-w64-mingw32-gcc")
    fi
  else
    _build_env+=("CC=gcc")
  fi

  log notice "Build environment: ${_build_env[*]}"
  log notice "Build arguments: ${_build_args[*]}"
  log notice "Output name: ${_output_name}"
  log notice "Main directory: ${_main_dir}"

  # Execute build
  if env "${_build_env[@]}" go build "${_build_args[@]}" -o "${_output_name}" "${_main_dir:-./cmd}"; then
    log success "Build successful. (Output: .${_output_name//${_ROOT_DIR:-}/})"
...
build_for_arch() {
  ...
  while IFS= read -r main_dir; do
    ...
    log notice "${build_count} Compiling binary: ${_output_name}"
  ...
  if [[ -f ${_output_name:-} ]]; then
    ...
    if [[ ${_is_interactive:-} != "true" || ${CI:-} != "true" ]]; then
      ...
      if [[ ${_force:-} =~ [yY] || ${_force:-} == "true" || ${NON_INTERACTIVE:-} == "true" ]]; then
        ...
        log notice "Binary already exists: ${_output_name:-}"
        log notice "Current binary: ${_output_name:-}"
        log notice "Press 'y' to overwrite or any other key to skip."
        log question "(y) to overwrite, any other key to skip (default: n, 10 seconds to respond)"
        ...
        log notice "Binary already exists: ${_output_name:-}"
        log notice "Skipping confirmation in non-interactive mode."
      fi
    fi

    if [[ ! ${REPLY:-} =~ [yY] ]]; then
      log notice "Skipping build for ${_platform_pos:-} ${_arch_pos:-}."
      return 0
    fi

    log warn "Overwriting existing binary: ${_output_name}"
    if [[ ${_platform_pos:-} == "windows" ]]; then
    ...
    if compile_binary "${_platform_pos:-}" "${_arch_pos:-}" "${_output_name:-}" "${_force:-}" "${_will_upx_pack_binary:-}"; then
      ...
      log error "Failed to build binary: ${_output_name}"
      return 1
    fi
  fi
...
arch_iterator() {
  ...
  local _main_dirs
  _main_dirs=$(discover_main_packages "${_root_dir:-.}/cmd/...")
  ...
  case "$(uname -s)" in
    Linux*)
      if have apt-get; then
        ...
        sudo yum install -y upx-ucl
      elif have dnf; then
        sudo dnf install -y upx-ucl
      elif have pacman; then
        sudo pacman -S --noconfirm upx-ucl
      else
